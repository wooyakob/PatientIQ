...
def clear_patient_collection(db: CouchbaseDB, patient_id: str, collection_name: str) -> int:
    """
    ...
    try:
        ...
        print(f"    ⚠️  No primary index found, trying collection scan...")
        try:
            ...
            print(f"    → Proceeding with load (old docs will remain if they exist)")
            return 0
...
def load_wearable_data_for_patient(db: CouchbaseDB, patient_id: str, json_file: Path) -> tuple[int, int]:
    """
    ...
    if deleted_count > 0:
        ...
        print(f"    → Collection cleared (or was empty)")
...
def main():
    ...
    patient_dirs = sorted([d for d in data_dir.iterdir() if d.is_dir() and d.name.startswith("patient_")])
    ...
    if total_errors == 0:
        ...
        print(f"\n📊 Next steps:")
        print(f"  1. Verify data: Check Couchbase UI → Wearables scope → Patient collections")
        print(f"  2. Run: python scripts/populate_wearable_vectors.py")
        print(f"  3. Test: make run-api and test the analytics endpoint")
