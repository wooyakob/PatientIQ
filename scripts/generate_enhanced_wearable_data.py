...
def generate_enhanced_patient_data(
    ...
    stress_patterns = {
        ...
        "escalating": lambda day: 1 if day < 10 else (2 if day < 20 else 3)  # Stress increases over time
    }
...
def main():
    ...
    for patient in patients:
        ...
        print(f"  Stats:")
        print(f"    Stress distribution: Low={stress_counts['Low']}, Med={stress_counts['Medium']}, High={stress_counts['High']}")
    ...
    print(f"  • Patient 1: Escalating stress → worsening asthma symptoms")
    print(f"  • Patient 2: High stress → low O2 with COPD")
    print(f"  • Patient 3: Moderate stress with pulmonary fibrosis")
    print(f"  • Patient 4: LOW stress → better asthma control (good comparison)")
    print(f"  • Patient 5: Moderate stress with COPD")
    print(f"\nThese patterns will:")
    print(f"  ✓ Show clear stress-symptom correlations")
    print(f"  ✓ Be easy to find via vector similarity search")
    print(f"  ✓ Correlate with medical research on stress & respiratory disease")
    print(f"  ✓ Make clinical sense for pulmonology")
    print(f"\n{'=' * 80}\n")
