...
def compare_patient_to_cohort(
    ...
    valid_patient_data = [
        d for d in patient_wearable_data 
        if "metrics" in d and isinstance(d.get("metrics"), dict)
    ]
    ...
    for metric_key, (short_name, display_name, unit, higher_is_better) in metric_mapping.items():
        ...
        if cohort_std > 0:
            ...
            if abs(z_score) > 1.5:
                ...
                if (higher_is_better and z_score < -1.5) or (not higher_is_better and z_score > 1.5):
                ...
                outliers.append({
                    ...
                    "difference_percent": round((difference / cohort_mean) * 100, 1) if cohort_mean != 0 else 0,
