...
def _create_trend_text(wearable_data: list[dict], patient_condition: Optional[str] = None) -> str:
    ...
    if heart_rates:
        ...
        if avg_hr > 100:
            parts.append(f"Elevated heart rate averaging {avg_hr:.0f} BPM with peaks at {max_hr:.0f} BPM.")
        elif avg_hr < 60:
            parts.append(f"Low heart rate averaging {avg_hr:.0f} BPM.")
        else:
            parts.append(f"Normal heart rate averaging {avg_hr:.0f} BPM ranging from {min_hr:.0f} to {max_hr:.0f} BPM.")
    ...
    if oxygen_levels:
        ...
        if min_o2 < 90:
            parts.append(f"Critical oxygen desaturation with minimum {min_o2:.1f}% and {days_below_92} days below 92%.")
        elif avg_o2 < 92:
            parts.append(f"Low oxygen saturation averaging {avg_o2:.1f}% with {days_below_92} days below normal.")
        else:
    ...
    if steps_counts:
        ...
        if avg_steps < 3000:
            parts.append(f"Very low activity with average {avg_steps:.0f} steps per day and {low_activity_days} days below 3000 steps.")
        elif avg_steps < 5000:
    ...
    if stress_levels:
        ...
        if avg_stress >= 2.5:
            ...
            parts.append(f"Moderate stress levels.")
        else:
            parts.append(f"Low stress levels.")
...
def vectorize_wearable_trends(
    ...
    for record in valid_data:
        ...
        heart_rates.append(float(metrics.get("heart_rate", 0)) if metrics.get("heart_rate") else None)
        oxygen_levels.append(float(metrics.get("blood_oxygen_level", 0)) if metrics.get("blood_oxygen_level") else None)
        ...
        exercise_durations.append(float(metrics.get("exercise_duration", 0)) if metrics.get("exercise_duration") else None)
        calories.append(float(metrics.get("calories_burned", 0)) if metrics.get("calories_burned") else None)
    ...
    if any(o2 is not None for o2 in oxygen_levels):
        ...
        if clean_o2:
            normalized_metrics["blood_oxygen_level"] = _normalize_metric(clean_o2, "blood_oxygen_level")
    ...
    if any(e is not None for e in exercise_durations):
        ...
        if clean_ex:
            normalized_metrics["exercise_duration"] = _normalize_metric(clean_ex, "exercise_duration")
